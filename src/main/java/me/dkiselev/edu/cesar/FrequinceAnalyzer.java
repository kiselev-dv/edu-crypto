...
import me.dkiselev.edu.util.DoubleCount;
...
public class FrequinceAnalyzer {
	...
	public static final double[] ru_freq = new double[]{
	...
	public static final double[] en_freq = new double[]{ 0.12702, 0.09056, 0.08167, 0.07507, 0.06966, 
	...
	public int decrypt(BufferedReader reader) throws IOException {
		...
		double[] textFreq = countTextFrequence(inputString);
		
		List<DoubleCount> correlations = countSpectralDistancesByOffset(textFreq, refFerq);
		
		return correlations.get(0).offset;
	}

	public double[] countTextFrequence(String inputString) {
	...
	public List<DoubleCount> countSpectralDistancesByOffset(double[] textFreq, double[] refFerq) {
		
		List<DoubleCount> result = new ArrayList<DoubleCount>();
		
		for(int i = 0; i < textFreq.length; i++) {
			DoubleCount doubleCount = new DoubleCount();
			doubleCount.val = countCorrelation(rotate(textFreq, i), refFerq);
			doubleCount.offset = i;
			result.add(doubleCount);
		}
		
		Collections.sort(result, new Comparator<DoubleCount>() {

			public int compare(DoubleCount o1, DoubleCount o2) {
